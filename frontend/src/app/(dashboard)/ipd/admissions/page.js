...
export default function AdmissionsPage() {
  ...
  async function loadAdmissions(isRefresh = false) {
    try {
      if (isRefresh) {
        setRefreshing(true);
      } else {
    ...
    <div className="space-y-7 p-4 sm:p-6 lg:p-8">
      <header className="flex flex-col justify-between gap-5 xl:flex-row xl:items-center">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-slate-900">Admissions</h1>

          <p className="mt-2 text-slate-600">
            Manage pending admissions and current inpatient records.
          </p>
        </div>
          ...
          <table className="min-w-[1100px] w-full">
            <thead className="bg-slate-50">
              <tr>
                ...
                <TableHeader>Ward / Room / Bed</TableHeader>
                ...
                <tr key={admission.id} className="hover:bg-slate-50">
                  ...
                  <TableCell>
                    <div>
                      <p className="font-semibold text-slate-800">
                        {admission.patient_detail?.name || "Unknown"}
                      </p>

                      <p className="mt-1 text-xs text-slate-500">
                        {admission.patient_detail?.phone || "No phone"}
                      </p>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <span className="capitalize">
                      {String(admission.admission_type || "").replaceAll(
                        "_",
                        " ",
                      )}
                    </span>
                  ...
                  <TableCell>
                    <div>
                      <p>{admission.ward_detail?.name || "No ward"}</p>

                      <p className="mt-1 text-xs text-slate-500">
                        Room: {admission.room_detail?.name || "None"}
                        {" • "}
                        Bed: {admission.bed_detail?.name || "None"}
                      </p>
                    </div>
                  </TableCell>

                  <TableCell>
                    {dateValue(admission.admitted_at || admission.created_at)}
                  </TableCell>
